...
const fetchFigma = (path) => fetch(`https://api.figma.com/v1/files/${FIGMA_FILE_KEY}${path}`, {
  headers: {
    'X-FIGMA-TOKEN': TOKEN,
  },
...
const rgbaToHex = (r, g, b, a) => {
  ...
  return `#${hr.length === 1 ? `0${hr}` : hr}${hg.length === 1 ? `0${hg}` : hg}${hb.length === 1 ? `0${hb}` : hb}${ ha.length === 1 ? `0${ha}` : ha}`;
}
const main = async () => {
  ...
  Object.values(nodes)
    ...
    .forEach(({ document }) => {
      ...
      semanticColors[colorName.replaceAll(' ', '')] = {
        value: !reference ? rgbaToHex(r * 255, g * 255, b * 255, opacity) : `{${reference.replaceAll(' ', '.')}.value}`
      }
    });
  ...
  await writeFile(path.resolve(__dirname, '../tokens/color/primitive.json'), primitiveContent);
  await writeFile(path.resolve(__dirname, '../tokens/color/semantics.json'), semanticsContent);
  console.log('DONE');
}
